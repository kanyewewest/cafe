...
export default function useRainbowText(
  $el: React.RefObject<HTMLElement | null>,
) {
  const play = (): void => {
    gsap.registerPlugin(SplitText);

    const lerpH = gsap.utils.interpolate([
      0, 50, 0, -140, -160, -250, -250,
    ]);
    const lerpS = gsap.utils.interpolate([100, 70, 80, 60]);
    const lerpL = gsap.utils.interpolate([
      70, 70, 15, 30, 65, 15, 0,
    ]);

    const ds = 1 / (COLOR_STEPS - 1); // Derivated steps
    const DEF_COLORS = new Array(COLOR_STEPS)
      .fill(undefined)
      .map(
        (_, i) =>
    ...
    const { lines } = new SplitText($el.current, {
      type: "lines, words",
    });

    for (let i = 0, cc = 0; i < lines.length; i++) {
      // cc: Total char count
      const { chars } = new SplitText(lines[i], {
        type: "chars",
      });
